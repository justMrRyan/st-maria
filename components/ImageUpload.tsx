...
export default function ImageUpload({
  ...
  const handleFileSelect = async (
    ...
    // Validate file size (optional – we'll compress anyway)
    ...
    try {
      ...
      const options = {
        ...
        maxWidthOrHeight: 1200,   // max dimension in pixels
        useWebWorker: true,
      ...
      // Upload the compressed file (Blob) to Supabase Storage
      const { error: uploadError } = await supabase.storage
        .from('MERYAMSWILEM')
        .upload(filePath, compressedFile, {
          cacheControl: '3600',
          upsert: false,
          contentType: compressedFile.type, // important to set correct MIME
        });
      ...
      toast.success('Image uploaded successfully (compressed)');
